#!/usr/bin/env python3
"""
Observatorio Economico de Caratinga - Pipeline de Dados
...
# ─── Configuracao ─────────────────────────────────────────────────────────────
MUNICIPIO_CODE = 3113404
MUNICIPIO_STR  = "3113404"
...
def try_http(url, timeout=60):
    ...
    try:
        ...
        r = requests.get(
            ...
            headers={"User-Agent": "ObservatorioCaratinga/1.1"}
        )
...
def try_tabelas_abertas(year, month):
    ...
    Retorna dict com adm/des/saldo/estoque para Caratinga ou None.
    ...
    Caratinga IBGE: 311340 (sem digito verificador, formato da Tabela 3).
    """
    try:
        import openpyxl
    except ImportError:
    ...
    try:
        ...
        # Caratinga aparece como "311340" nas tabelas (sem digito verificador)
        CARATINGA_CODES = {"311340", "3113404", "mg-caratinga", "caratinga"}

        for row in sheet.iter_rows(values_only=True):
            # Verifica se alguma celula identifica Caratinga
            row_str = [str(c).strip().lower() if c is not None else "" for c in row]
            if not any(code in row_str for code in CARATINGA_CODES):
                # Tenta verificacao parcial
                if not any("caratinga" in cell for cell in row_str):
                    continue
            ...
            if len(clean_nums) >= 6:
                ...
                log.info(
                    f"Tabelas abertas OK — Caratinga {ym}: "
        ...
        log.warning("Tabelas abertas: linha de Caratinga nao encontrada na Tabela 3.")
    except Exception as exc:
...
def parse_caged_csv(csv_text):
    """
    Le o CSV do CAGED, filtra Caratinga e produz o dict para o dashboard.
    ...
    if not chunks:
        log.warning("Nenhum registro para Caratinga neste arquivo.")
        return None

    df = pd.concat(chunks, ignore_index=True)
    log.info(f"Registros de Caratinga: {len(df)}")
...
def main():
    log.info("=== Observatorio Economico de Caratinga - Pipeline de dados ===")
    ...
    if caged_result:
        ...
        meta["sources"]["caged"] = {
            ...
            "reference_month_label": label,
            "downloaded_at": now,
            "records_caratinga": caged_result.get("total_records", 0),
            "status": "ok",
